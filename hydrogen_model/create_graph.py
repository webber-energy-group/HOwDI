...
def initialize_graph(H):
    ...
    for num, arow in H.arcs.iterrows():
        ...
        capital_price_multiplier = (
            H.nodes[(H.nodes["node"] == start_node) | (H.nodes["node"] == end_node)][
                "capital_pm"
            ].sum()
            / 2
        )
    ...
    for num, arow in H.nodes.iterrows():
        ...
        for demand_sector in H.demand["sector"]:
            ...
            if arow["%s_tonnesperday" % demand_sector] == 0:
                ...
                demand_node_char["size"] = arow["%s_tonnesperday" % demand_sector] * (
...
def add_price_nodes(g: DiGraph, H):
    ...
    if not H.find_prices:
        ...
        if H.price_hubs == "all":
            H.price_hubs = set([s[1] for s in list(g.nodes(data="hub_name"))])
        for ph in H.price_hubs:
            ...
            for p in H.price_tracking_array:
                ...
                for demand_type in ["fuelStation", "lowPurity", "highPurity"]:
                    price_node_dict = {
                        "node": ph + "_price{}_{}".format(cap_first(demand_type), p),
                        "sector": "price",
                        "hub_name": ph,

...
def discrete_pdf(demandSector, demand_csv):
    ...
    discrete_price_points = numpy.linspace(mean-2*std, mean+2*std, discrete_consumers)
    ...
    consumers_regular = pandas.DataFrame({'sector': demandSector, 
    ...
    consumers_carbon = pandas.DataFrame({'sector': demandSector, 
                      'firm': [s + '_c' for s in list(discrete_price_points.round(0).astype(int).astype(str))], 
                      'breakevenPrice': discrete_price_points.round(0), 
...
def node_discrete_consumers(node, consumers_df, nodes_csv):
    ...
    '''
    node_consumers_add =  pandas.concat([pandas.DataFrame({'node': numpy.repeat(node, len(consumers_df))}), consumers_df.copy()], axis=1)
    node_sector_size_list = list(nodes_csv[nodes_csv['node']==node][consumers_df['sector']].iloc[0])
...
def main(H):
    ...
    consumers = pandas.DataFrame(columns = ['sector', 'firm', 'breakevenPrice', 'size', 'carbonSensitive', 'breakevenCarbon_g_MJ', 'fuelingStation'])
    node_consumers = pandas.DataFrame(columns = ['node', 'sector', 'firm', 'breakevenPrice', 'size', 'carbonSensitive', 'breakevenCarbon_g_MJ', 'fuelingStation'])
    ...
    for n in nodes_csv['node']:
        node_consumers = pandas.concat([node_consumers, node_discrete_consumers(n, consumers, nodes_csv)])
        node_consumers = node_consumers.reset_index(drop=True)
    #add existing demand
    node_consumers = add_existing_demand(node_consumers, consumers_existing)
    #index and return
    node_consumers['name'] = node_consumers['node'] + '_' + node_consumers['sector'] + '_' + node_consumers['firm']
